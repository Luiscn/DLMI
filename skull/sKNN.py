...
os.chdir('/Users/luis/Documents/MedImag/skull/skull_no_shift')
...
for i in range(len(data_test)):
    ...
    for j in range(len(data_train)):
        ...
        ind[i,:] = np.argsort(dist)[:K]
        skull_diff[i,:] = np.sort(dist)[0:1]
    

mag_train = label_mag[:,:,:900]
mag_test = label_mag[:,:,900:]
phz_train = np.moveaxis(phz_in[:900,:,:], 0, -1)
phz_test = np.moveaxis(phz_in[900:,:,:], 0, -1)

K_f = 2 # k nearest fields

# Near neighbors to have less magnitude differences
#ind_f = np.zeros((np.shape(mag_test)[2], K_f))
#field_diff = np.zeros((np.shape(mag_test)[2], K_f))
#for i in range(np.shape(mag_test)[2]):
#    dist_f = np.zeros(np.shape(mag_train)[2])
#    for j in range(np.shape(mag_train)[2]):
#        dist_f[j] = sDist(mag_test[:,:,i], mag_train[:,:,j])
#        ind_f[i,:] = np.argsort(dist_f)[:K_f]
#        field_diff[i,:] = np.sort(dist_f)[0:1]

field_train = mag_train * np.exp(1j*phz_train)
field_test = mag_test * np.exp(1j*phz_test)

# Near neighbors to have large cosine similarities
...
for i in range(np.shape(field_test)[2]):
    ...
    for j in range(np.shape(field_train)[2]):
        dist_f[j] = cos_corr(field_test[:,:,i], field_train[:,:,j])
        ind_f[i,:] = np.flip(np.argsort(dist_f)[-K_f:], axis = 0)
        field_diff[i,:] = np.sort(dist_f)[0:1]
